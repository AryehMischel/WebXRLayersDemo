...
async function loadFilesInFolder(source, fileExtensions) {
  ...
  let layer = new WebXRCubeLayerASTC(
    loadedFiles,
    source.width,
    source.height,
    true
...
async function createQuadIU() {
  ...
  let layer = new WebXRQuadUILayer(
    canvasImage,
    "canvasQuad",
    quadUIscaleWidth,
    quadUIscaleHeight,
...
function addColliders() {
  ...
  loader.load("./Assets/Images/border.png", function (texture) {
    ...
    battleCollider.position.set(
      mapValueWidth(battleX),
      mapValueHeight(battleY),
      layerDepth
    ); //battlePosition
    forestCollider.position.set(
      mapValueWidth(forestX),
      mapValueHeight(forestY),
      layerDepth
    );
    dreamCollider.position.set(
      mapValueWidth(dreamX),
      mapValueHeight(dreamY),
      layerDepth
    );
  });
}
...
function mapValueWidth(input) {
  ...
  // lerp
  const mappedValue =
    ((input - canvasMin) / (canvasMax - canvasMin)) *
      (boxPositionMax - boxPositionMin) +
    boxPositionMin +
    quadUIpositionX;
...
class WebXRCubeLayer {
  ...
  createLayer(texture = this.Cube_Texture) {
    ...
    console.log(
      "height, widht",
      texture.source.data[0].width,
      texture.source.data[0].height
    );
